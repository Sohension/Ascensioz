    ...
    >
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex justify-between items-center h-16">
          ...
          {/* Desktop Links (Left/Natural flow) */}
                ...
                >
                  {link.href ? (
                    ...
                    >
                      {link.name}
                      {!link.comingSoon && (
                        <span className="absolute left-0 -bottom-1 h-0.5 w-full bg-black dark:bg-white scale-x-0 transition-transform duration-300 ease-in-out group-hover:scale-x-100 origin-left"></span>
                      )}
                    </Link>
                  ) : (
                    <span className="tracking-wide cursor-pointer transition-colors duration-200 hover:text-gray-600 dark:hover:text-gray-300 flex items-center gap-1">
                      {link.name}
                    </span>
                  )}
                  ...
                  {hasDropdown && (
                    ...
                    >
                      <div className="rounded-xl shadow-xl bg-white/90 dark:bg-zinc-900/95 backdrop-blur-3xl border border-white/20 dark:border-white/10 flex flex-col p-2">
                        {link.children?.map((child) => (
                          <Link
                            key={child.name}
                            href={child.href}
                            className="px-3 py-2 rounded-md transition-colors duration-200 hover:bg-black/5 dark:hover:bg-white/10 text-gray-700 dark:text-gray-200"
                          >
                            {child.name}
                          </Link>
                        ))}
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <button
                ...
                className="relative w-9 h-9 rounded-full overflow-hidden cursor-pointer ring-2 ring-gray-200 dark:ring-zinc-800 hover:ring-black dark:hover:ring-white transition-all duration-200 focus:outline-none flex items-center justify-center bg-gray-100 dark:bg-zinc-800"
              ...
              <div
                className={`absolute right-0 top-full mt-1 w-44 rounded-xl shadow-xl bg-white/90 dark:bg-zinc-900/95 backdrop-blur-3xl border border-white/20 dark:border-white/10 transition-all duration-200 ease-in-out z-50 ${
                  isProfileOpen
                    ? "opacity-100 translate-y-0 visible"
                    : "opacity-0 -translate-y-2 invisible"
                }`}
              >
                <div className="flex flex-col p-2 text-sm font-medium">
                  {user ? (
                    <>
                      <div className="px-3 py-1.5 text-xs text-gray-400 border-b border-gray-100 dark:border-zinc-800 mb-1 truncate">
                        {user.email}
                      </div>
                      ...
                      <button
                        ...
                        className="text-left w-full px-3 py-2 rounded-lg transition-colors duration-200 hover:bg-red-50 dark:hover:bg-red-950/30 text-red-600 dark:text-red-400 font-semibold"
                      >
                        Logout
                      </button>
                    </>
      ...
      {isOpen && (
        <div className="md:hidden border-t border-white/20 dark:border-white/10 bg-white/90 dark:bg-zinc-950/95 backdrop-blur-3xl px-4 pt-2 pb-6 space-y-2">
          {navLinks.map((link) => {
            ...
            return (
              <div
                key={link.name}
                className="border-b border-gray-100/50 dark:border-zinc-800/50 last:border-none pb-2 pt-1"
              >
                {hasChildren ? (
                  <div>
                    <button
                      ...
                      <div className="pl-4 py-1 space-y-1 flex flex-col">
                        {link.children?.map((child) => (
                          <Link
                            ...
                            className="py-2 text-sm font-medium transition-colors duration-200 text-gray-600 dark:text-gray-400 hover:text-black dark:hover:text-white"
                          >
                            {child.name}
                          </Link>
                        ))}
      ...
      {toastMounted && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 pointer-events-none">
          <div
            className={`bg-white/90 dark:bg-zinc-900/95 backdrop-blur-2xl border border-white/30 dark:border-white/10 text-black dark:text-white px-5 py-2 rounded-xl shadow-xl text-sm transition-all duration-300 ease-in-out ${
              toastVisible
